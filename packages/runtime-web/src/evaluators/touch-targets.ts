...
const LIMB_JOINTS: Record<TouchTargetsChallenge["limb"], readonly string[]> = {
 ...
 */
export class TouchTargetsEvaluator implements ChallengeEvaluator {
  private readonly dwellGoal: number;
  private targetList: MutableTarget[] = [];
  private framing: ChallengeGuide["framing"] = null;
  private confidence = 0;

  constructor(
    private readonly challenge: TouchTargetsChallenge,
  ...
  enter(): void {
    this.targetList = createGridTargets(
      ...
      this.challenge.zone,
      this.challenge.targetScale * TIER_PROFILES[this.ctx.tier].targetScale,
      this.ctx.reachBox,
  ...
  guide(): ChallengeGuide {
    ...
    const activeDwell = this.targetList.reduce(
      (best, target) => target.hit ? best : Math.max(best, target.dwellMs / Math.max(1, this.dwellGoal)),
      0,
    );
    return {

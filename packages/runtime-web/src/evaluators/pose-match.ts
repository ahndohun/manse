...
import { mirrorJointName, type BodyMetricsSample, type JointAngleName } from "../body-metrics.js";
...
type PoseMatchConfig = Extract<SequenceStep, { type: "pose_match" }>;
 ...
 */
export class PoseMatchEvaluator implements ChallengeEvaluator {
  private readonly noiseScale: number;
  private cursor = 0;
  private heldMs = 0;
  private matchedNow = false;
  private mirroredBest = false;
  private lastSample: BodyMetricsSample | null = null;
  private framing: ChallengeGuide["framing"] = null;
  private confidence = 0;

  constructor(
    private readonly config: PoseMatchConfig,
  ...
  private holdGoalMs(pose: PoseMatchConfig["poses"][number]): number {
    return Math.max(1, pose.holdMs * TIER_PROFILES[this.ctx.tier].dwellScale);
  }

  private matchScore(
    sample: BodyMetricsSample,
    pose: PoseMatchConfig["poses"][number],
    mirrored: boolean,
  ): { ratio: number; judged: number } {
    let matched = 0;
    let judged = 0;
    for (const joint of pose.joints) {
      const name = mirrored ? mirrorJointName(joint.joint as JointAngleName) : (joint.joint as JointAngleName);
      const actual = sample.angles.get(name);
      if (actual === undefined) continue;
      judged += 1;
      if (Math.abs(actual - joint.angleDeg) <= joint.toleranceDeg * this.noiseScale) matched += 1;
    }
  ...
  private jointFeedback(
    sample: BodyMetricsSample,
    pose: PoseMatchConfig["poses"][number],
  ): readonly JointFeedback[] {
    return pose.joints.map((joint) => {
      const name = this.mirroredBest ? mirrorJointName(joint.joint as JointAngleName) : (joint.joint as JointAngleName);
      const actual = sample.angles.get(name);
 ...
 */
export function buildSilhouette(pose: PoseMatchConfig["poses"][number]): readonly SilhouetteSegment[] {
  const angle = (name: string, fallback: number): number => {
  ...
  const torso: Array<readonly [CanonicalPoint, CanonicalPoint]> = [
  ...
  for (const [a, b] of torso) segments.push({ x0: a.x, y0: a.y, x1: b.x, y1: b.y });
...
function rotate(vector: CanonicalPoint, degrees: number): CanonicalPoint {
...
function advance(from: CanonicalPoint, direction: CanonicalPoint, length: number): CanonicalPoint {

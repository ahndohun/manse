...
import type { RuntimeLandmark, RuntimePose, RuntimePoseFrame } from "./types.js";
...
export interface MotionKeyframe {
  ...
  readonly body?: { readonly dx?: number; readonly dy?: number };
...
const STANDING_COORDINATES: readonly (readonly [number, number, number])[] = [
...
export function standingLandmarks(): RuntimeLandmark[] {
...
export function synthesizePoseFrames(script: MotionScript, startSequence = 0): RuntimePoseFrame[] {
  ...
  for (let index = 0; index <= frameCount; index += 1) {
    ...
    if (timestampMs > script.durationMs) break;
    frames.push({
      timestampMs,
...
export function synthesizeMultiPoseFrames(
  scripts: readonly MotionScript[],
  fps: number,
  durationMs: number,
): RuntimePoseFrame[] {
...
function poseAt(keyframes: readonly MotionKeyframe[], timestampMs: number): RuntimePose {
...
function bracket(
  ...
  timestampMs: number,
): { before: MotionKeyframe | undefined; after: MotionKeyframe | undefined; t: number } {
  if (keyframes.length === 0) return { before: undefined, after: undefined, t: 0 };
  let before = keyframes[0];
  let after = keyframes[keyframes.length - 1];
  for (const keyframe of keyframes) {
    if (keyframe.atMs <= timestampMs) before = keyframe;

import { parseEpisodePack, type EpisodePack } from "@manse/schema";

import type { EpisodePackSource, LoadedEpisodePack, RuntimePlatform } from "./types.js";

const MAX_PACK_JSON_BYTES = 2 * 1024 * 1024;

export async function loadEpisodePack(
  source: EpisodePackSource,
  platform: RuntimePlatform,
): Promise<LoadedEpisodePack> {
...
async function loadEpisodePackUrl(
  source: string | URL,
  platform: RuntimePlatform,
): Promise<LoadedEpisodePack> {
...
export function resolvePackAssetUrl(
  ...
  if (asset === undefined) throw new Error(`Unknown pack asset '${assetId}'.`);
  return resolveSafeRelativeUrl(asset.path, baseUrl);
}

export function resolveSafeRelativeUrl(path: string, baseUrl: URL): URL {
  if (
    path.trim() === "" ||
    path.startsWith("/") ||
    path.startsWith("\\") ||
...
function assertLocalPackUrl(url: URL, platform: RuntimePlatform): void {
...
function isWrappedPackSource(
  value: EpisodePack | { readonly pack: unknown; readonly baseUrl?: URL | string },
): value is { readonly pack: unknown; readonly baseUrl?: URL | string } {

...
import type { CommandResult, WritableOutput } from "./types.js";
...
function parseInvocation(argv: readonly string[]): Invocation {
  ...
  if (command === undefined) return { args: [], command: "help", json: jsonCount === 1 };
...
): { options: Record<string, string | boolean | undefined>; positionals: string[] } {
...
function requirePositionals(positionals: readonly string[], count: number, usage: string): void {
...
async function execute(
  ...
  switch (invocation.command) {
    ...
    case "doctor": {
      ...
      if (options.help === true) return { data: { help: "Usage: manse doctor [--json]" }, message: "Usage: manse doctor [--json]" };
    ...
    case "validate": {
      const { options, positionals } = parseCommandArgs(invocation.args);
      if (options.help === true) return { data: { help: "Usage: manse validate <project-or-pack>" }, message: "Usage: manse validate <project-or-pack>" };
    ...
    case "manifest": {
      const { options, positionals } = parseCommandArgs(invocation.args);
      if (options.help === true) return { data: { help: "Usage: manse manifest <project>" }, message: "Usage: manse manifest <project>" };
    ...
    case "catalog": {
      ...
      if (operation === "add") {
        ...
        if (options.help === true) return { data: { help: "Usage: manse catalog add <manifest-url> --catalog <path>" }, message: "Usage: manse catalog add <manifest-url> --catalog <path>" };
      ...
      if (operation === "build") {
        ...
        if (options.help === true) return { data: { help: "Usage: manse catalog build <catalog> --out <path>" }, message: "Usage: manse catalog build <catalog> --out <path>" };
    ...
    case "pack": {
      ...
      if (options.help === true) return { data: { help: "Usage: manse pack <project> --out <archive.zip-or-directory>" }, message: "Usage: manse pack <project> --out <archive.zip-or-directory>" };
...
export async function runCli(argv: readonly string[], dependencies: CliDependencies = {}): Promise<number> {
  ...
  try {
    ...
    if (json) writeJsonResult(stdout, "unknown", false, { diagnostics: cliError.diagnostics }, cwd);
    else {
      for (const diagnostic of cliError.diagnostics) stderr.write(`${formatDiagnostic(diagnostic, cwd)}\n`);
    }

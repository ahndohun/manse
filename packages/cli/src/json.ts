...
function sortJson(value: unknown, seen = new Set<object>(), pointer = ""): unknown {
  ...
  if (Array.isArray(value)) {
    if (seen.has(value)) throw new CliError("CYCLIC_JSON", "Cannot serialize a cyclic value.", { exitCode: ExitCode.Internal });
    seen.add(value);
...
export async function readJsonFile(filePath: string): Promise<unknown> {
  ...
  try {
    ...
    if (stat.size > MAX_JSON_BYTES) {
      throw new CliError(
        "JSON_TOO_LARGE",
        `JSON files may not exceed ${MAX_JSON_BYTES} bytes: ${filePath}`,
        { exitCode: ExitCode.LocalIo },
      );
    }

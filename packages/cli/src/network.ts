...
export const defaultNetworkDependencies: NetworkDependencies = {
...
export function parsePublicManifestUrl(value: string): URL {
  let url: URL;
  try {
    url = new URL(value);
  } catch (error) {
  ...
  if (url.username !== "" || url.password !== "" || url.search !== "" || url.hash !== "") {
    throw new CliError(
      "UNSAFE_MANIFEST_URL",
      "Manifest URLs may not contain credentials, query parameters, or fragments.",
      { exitCode: ExitCode.Network },
    );
  }
  ...
  if (url.pathname !== MANIFEST_URL_PATH) {
    throw new CliError(
      "WRONG_MANIFEST_PATH",
      `Manifest URLs must use the exact path ${MANIFEST_URL_PATH}.`,
      { exitCode: ExitCode.Network },
    );
  }
...
async function assertPublicResolution(url: URL, resolveHostname: ResolveHostname): Promise<void> {
  ...
  if (addresses.length === 0 || addresses.some(isPrivateAddress)) {
    throw new CliError(
      ...
      "Manifest URLs may not resolve to private, local, or reserved network addresses.",
      { exitCode: ExitCode.Network },
    );
  }
}

function getUrlField(manifest: Record<string, unknown>, key: string): string | undefined {
...
function assertManifestOriginPolicy(manifest: Record<string, unknown>, manifestUrl: URL): void {
  ...
  if (gameUrlValue !== undefined) {
    ...
    if (gameUrl.protocol !== "https:" || gameUrl.origin !== manifestUrl.origin) {
      throw new CliError(
        "GAME_ORIGIN_MISMATCH",
        "The manifest gameUrl must use HTTPS and the same Site origin as the manifest.",
        { exitCode: ExitCode.Network },
      );
    }
  }
  ...
  if (thumbnailValue !== undefined) {
    ...
    if (thumbnailUrl.protocol !== "https:" || thumbnailUrl.origin !== manifestUrl.origin) {
      throw new CliError(
        "THUMBNAIL_ORIGIN_MISMATCH",
        "The public thumbnail must be served from the same Site origin as the manifest.",
        { exitCode: ExitCode.Network },
      );
    }
...
async function fetchSameOriginResource(
  rawUrl: string,
...
export async function fetchPublicManifest(
  rawUrl: string,
  dependencies: NetworkDependencies = defaultNetworkDependencies,
  ...
  for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount += 1) {
    ...
    if (gameUrl !== undefined) await fetchSameOriginResource(gameUrl, initialUrl.origin, "game", dependencies);

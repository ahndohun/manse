...
function assertPublishablePath(relativePath: string): void {
...
async function assertNoRuntimeCdn(deployRoot: string): Promise<void> {
  ...
  for (const match of html.matchAll(resourceTag)) {
    ...
    if (/^(?:https?:)?\/\//iu.test(resource)) {
      throw new CliError(
        "RUNTIME_CDN_BLOCKED",
        `The built Site references a runtime CDN resource (${resource}). Bundle runtime assets into the Site before packing.`,
  ...
  if (/\b(?:@import|url\()\s*["']?(?:https?:)?\/\//iu.test(html)) {
    throw new CliError(
      "RUNTIME_CDN_BLOCKED",
      "The built Site contains a remote CSS runtime resource. Bundle it into the Site before packing.",
      { exitCode: ExitCode.Validation },
    );
  }
}

async function findDeployRoot(projectRoot: string): Promise<string> {
  ...
  if (candidates.length === 0) {
    throw new CliError(
      "DEPLOY_ROOT_NOT_FOUND",
      "No built static Site was found. Build the project first; the output must contain index.html and .well-known/manse-game.json.",
      { exitCode: ExitCode.LocalIo },
    );
  }
  if (candidates.length > 1) {
    throw new CliError(
      "AMBIGUOUS_DEPLOY_ROOT",
      `More than one built Site was found (${candidates.join(", ")}). Remove stale outputs and try again.`,
      { exitCode: ExitCode.Conflict },
    );
  }
  const deployRoot = candidates[0];
  if (deployRoot === undefined) throw new CliError("DEPLOY_ROOT_NOT_FOUND", "No built static Site was found.");
  return deployRoot;
}
...
async function createDeterministicZip(files: readonly PublicationFile[]): Promise<Buffer> {
...
async function createDirectoryArtifact(
  outputPath: string,
...
export async function packPublication(
  ...
  for (const file of walked) assertPublishablePath(file.relativePath);

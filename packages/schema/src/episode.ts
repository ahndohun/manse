...
function validateContractVersion(pack: EpisodePack, issues: IntegrityIssue[]): void {
  if (pack.schemaVersion === 1) {
    if (pack.meta.players !== undefined) {
      issues.push(makeIssue(
        "schema_version",
        ["meta", "players"],
        "Multiplayer declarations require pack schemaVersion 2",
      ));
    }
    pack.scenes.forEach((scene, sceneIndex) => {
      if (scene.challenge !== null && scene.challenge.type !== "touch_targets") {
        issues.push(makeIssue(
          "schema_version",
          ["scenes", sceneIndex, "challenge", "type"],
          `Challenge type '${scene.challenge.type}' requires pack schemaVersion 2`,
        ));
      }
      scene.transitions.forEach((transition, transitionIndex) => {
        if (transition.adapt === null) return;
        for (const key of V2_ADAPT_KEYS) {
          if (transition.adapt[key] !== undefined) {
            issues.push(makeIssue(
              "schema_version",
              ["scenes", sceneIndex, "transitions", transitionIndex, "adapt", key],
              `Adaptation '${key}' requires pack schemaVersion 2`,
            ));
          }
        }
      });
  ...
  if (compareSemver(pack.meta.engine.minimumVersion, MOTION_CONTRACT_ENGINE_VERSION) < 0) {
    issues.push(makeIssue(
      "engine_compatibility",
      ["meta", "engine", "minimumVersion"],
      `schemaVersion 2 packs require engine.minimumVersion >= ${MOTION_CONTRACT_ENGINE_VERSION} so 0.1 runtimes reject them cleanly`,
    ));
  }
}

function validateChallengeShape(
  challenge: NonNullable<Scene["challenge"]> | SequenceStep,
  path: Array<string | number>,
  issues: IntegrityIssue[],
): void {
  ...
  if (challenge.type === "pose_match") {
    ...
    challenge.poses.forEach((pose, poseIndex) => {
      for (const duplicate of duplicateValues(pose.joints.map((joint) => joint.joint))) {
        issues.push(makeIssue(
          "duplicate_id",
          [...path, "poses", poseIndex, "joints"],
          `Joint '${duplicate}' is listed more than once in pose '${pose.id}'`,
        ));
      }
    });
  }

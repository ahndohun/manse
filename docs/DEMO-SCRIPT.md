...
| --- | --- | --- |
| 0:00–0:12 | A player completes two targets in `<OWNER_GAME_URL>`; cut to the Manse Showcase. | “This is Manse: an open-source engine and publishing ecosystem for camera-based active games. A normal link turns a phone, computer, tablet, or compatible display into the play surface.” |
...
| 1:48–2:08 | Publish confirmation, then open `<OWNER_GAME_URL>` and `/.well-known/manse-game.json` signed out. | “With my approval, Codex packages the self-contained game and publishes an independent public Site. Runtime code, models, WebAssembly, packs, and media are bundled; play needs no API key or CDN.” |
